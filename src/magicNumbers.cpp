...
void MagicNumbers::EPSWriteProtonNumber(std::ofstream &outFile, const int number) const
{
  ...
  const int min = ( position.min_n - Nmin < 0 )
    ? min_val
    : position.min_n - Nmin;

  const int max = ( (position.max_n - Nmin) > (Nmax - Nmin + max_val) )
    ? Nmax - Nmin + max_val
    : position.max_n - Nmin;
...
void MagicNumbers::EPSWriteNeutronNumber(std::ofstream &outFile, const int number) const
{
  ...
  const int min = ( position.min_z - Zmin < 0 )
    ? min_val
    : position.min_z - Zmin;

  const int max = ( position.max_z - Zmin > (Zmax - Zmin + max_val) )
    ? Zmax - Zmin + max_val
    : position.max_z - Zmin;
