...
TEST_CASE("EPS colours", "[EPSChart]")
{
  Options options;
  const EPSChart theChart(options);

  const std::string colours = fmt::format("/black     {{0 0 0}} bd\n"
                                          "/white     {{1 1 1}} bd\n"
                                          "/red       {{1 0 0}} bd\n"
                                          "/green     {{0 1 0}} bd\n"
                                          "/blue      {{0 0 1}} bd\n"
...
TEST_CASE("EPS isotope", "[EPSChart]")
{
  Options options;
  const EPSChart theChart(options);

  const std::string isotope{ "/half{\n"
                             "rotate\n"
                             "black rgb\n"
                             "BoxWidth neg 0 m\n"
                             "0 BoxWidth neg rl\n"
  ...
  REQUIRE(theChart.isotope() == isotope);
}
...
TEST_CASE("EPS definitions", "[EPSChart]")
{
  ...
  const std::string definitions{ "gsave 45 dict begin\n\n"
