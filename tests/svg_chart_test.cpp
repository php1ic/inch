...
TEST_CASE("SVG prolog", "[SVGChart]")
{
  Options options;
  const SVGChart theChart(options);

  const std::string prolog = fmt::format(
  ...
  // Don't use Catch::Equals here, it throws a "Invalid range in bracket expression." error
  // This may be unicode or 16bit regex related.
