  ...
  if ( !dripFile )
    {
      std::cerr << "\n"
                << "***ERROR***: " << file
                << " couldn't be opened to write to"
                << "\n" << std::endl;
      return 2;
    }
          ...
          if ( currentIsotope->A - otherIsotope->A == 1 )
            {
              ...
              if ( the_line == LineType::singleneutron
		   && otherIsotope->Z == currentIsotope->Z
		   && currentSingleProton < currentIsotope->Z
              ...
              if ( the_line == LineType::singleproton
		   && otherIsotope->N == currentIsotope->N
		   && currentSingleNeutron < currentIsotope->N
          ...
          else if ( currentIsotope->A - otherIsotope->A == 2 )
            {
              ...
              if ( the_line == LineType::doubleneutron
		   && otherIsotope->Z == currentIsotope->Z
		   && currentDoubleProton < currentIsotope->Z
              ...
              if ( the_line == LineType::doubleproton
		   && otherIsotope->N == currentIsotope->N
  ...
  drip_file = [&]()
    {
      switch (the_line)
	{
	case LineType::singleneutron: return draw->neutron_drip;
	case LineType::doubleneutron: return draw->two_neutron_drip;
	case LineType::singleproton: return draw->proton_drip;
	case LineType::doubleproton: return draw->two_proton_drip;
	default: return std::string("NoFile");
	}
    }();
...
int DripLine::EPSWriteLine(std::ofstream &outFile) const noexcept
{
  ...
  if ( !drip )
    {
      std::cerr << "***ERROR***: " << drip_file
                << " couldn't be opened to read the drip line data"
		<< std::endl;
      return 1;
    }
  ...
  while ( getline(drip, line) )
    {
      ...
      if (   zDrip >= Zmin
          ...
          && nDrip <= Nmax)
        {
          outFile << std::setw(3) << nDrip - Nmin << " "
                  << std::setw(3) << zDrip - Zmin << " "
                  << (initial ? 'm' : 'l') << '\n';
